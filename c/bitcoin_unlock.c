...
int main(int argc, char* argv[])
{
  ...
  ret = secp256k1_context_initialize(&context, SECP256K1_CONTEXT_VERIFY);
  if (ret == 0) {
    return ERROR_SECP_INITIALIZE;
  }

  ret = hex_to_bin(buf, TEMP_BUFFER_SIZE, argv[1]);
  CHECK_LEN(ret);
    ...
    case SIGHASH_MULTIPLE:
      {
        ...
        ns(CellOutput_vec_t) outputs = ns(Transaction_outputs(tx));
        size_t outputs_len = ns(CellOutput_vec_len(outputs));
        const char* ptr = argv[4];
        size_t len = strlen(ptr);
        while (ptr - argv[4] < len) {
          char* end = NULL;
