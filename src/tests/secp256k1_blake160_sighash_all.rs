...
mod multisig_tests {
    ...
    fn pkh(key: &Privkey) -> Bytes {
        Bytes::from(blake160(&key.pubkey().unwrap().serialize()).as_bytes())
    }

    fn verify(data_loader: &DummyDataLoader, tx: &Transaction) -> Result<u64, ScriptError> {
        let resolved_tx = build_resolved_tx(&data_loader, &tx);
        let script_config = ScriptConfig::default();
    ...
    fn test_sighash_2_3_unlock() {
        ...
        {
            // add extra arg
            let raw_tx = TransactionBuilder::from_transaction(raw_tx.clone())
                .witness(vec![Bytes::from("x")])
                .build();
            let tx = multi_sign_tx(raw_tx, &[&keys[0], &keys[1]]);
            assert_eq!(3, tx.witnesses()[0].len());
            verify(&data_loader, &tx).expect("pass verification");
        }
